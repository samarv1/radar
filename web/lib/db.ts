...
export async function getFeed(): Promise<Company[]> {
  const { rows } = await pool.query<Company>(`
    ...
    -- a16z Build newsletter companies not matched to any accelerator.
    -- These are early-stage startups featured in the newsletter's hiring sections.
    -- No amount required; date is the newsletter publication date.
    SELECT * FROM (
      SELECT DISTINCT ON (fn.company_name)
        fn.id + 3000000 AS id,
        fn.company_name AS name,
        NULL::text    AS website,
        'a16z_build'::text AS accelerator,
        NULL::text    AS batch,
        NULL::text    AS careers_ats,
        NULL::text    AS careers_url,
        fn.created_at::text,
        ...
        0::int AS other_count
      FROM funding_news fn
      WHERE fn.source = 'a16z_build'
        AND fn.accelerator_id IS NULL
        AND NOT EXISTS (
          SELECT 1 FROM edgar_filings ef2
          WHERE ef2.accelerator_id IS NULL AND ef2.standalone_source IS NOT NULL
            AND LOWER(TRIM(ef2.company_name)) = LOWER(TRIM(fn.company_name))
        )
        AND NOT EXISTS (
          SELECT 1 FROM funding_news fn2
          WHERE fn2.source = 'techcrunch'
            AND LOWER(TRIM(fn2.company_name)) = LOWER(TRIM(fn.company_name))
        )
      ORDER BY fn.company_name, fn.published_at DESC
    ) a16z_build_only
